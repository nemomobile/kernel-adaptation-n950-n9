...
static inline int twl5031_usb_read(u8 reg)
{
	...
	ret = twl_i2c_read_u8(TWL4030_MODULE_USB, &val, reg);
	if (ret)
		return ret;
...
{
	return twl_i2c_write_u8(TWL4030_MODULE_USB, val, reg);
}
...
static int twl5031_bcc_usb_charger_type(struct twl5031_bcc_data *bcc)
{
	...
	/* TODO: suspend? otg_set_suspend(bcc->otg, 0); */
	...
	if ((reg & 3) != 3) {
		...
		/* TODO: suspend? otg_set_suspend(bcc->otg, 1); */
	}
...
static void twl5031_bcc_psy_fsm_detect(struct twl5031_bcc_data *bcc)
{
	...
	default:
		ctl &= ~TWL5031_SW_USBCHRG_EN;
		dev_err(bcc->dev, "No charger detected, STS %02x\n", res);
		break;
	}
...
static int __devinit twl5031_bcc_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(bcc->dev, "could not request irq (chg det)\n");
		goto request_irq_failed;
	}

	bcc->nb.notifier_call = bcc_notifier_call;
	ret = usb_register_notifier(bcc->otg, &bcc->nb);
	if (ret)
...
get_regulator_failed:
	 ...
	 */
err:
	dev_err(bcc->dev, "probe error. Bye \n");
...
static int __exit twl5031_bcc_remove(struct platform_device *pdev)
{
	...
	free_irq(platform_get_irq(pdev, 0), bcc);
	usb_unregister_notifier(bcc->otg, &bcc->nb);
	regulator_put(bcc->usb3v1);
	usb_put_transceiver(bcc->otg);
	power_supply_unregister(&bcc->usb);
	power_supply_unregister(&bcc->vac);
	destroy_workqueue(bcc->wq);
...
static int __init twl5031_bcc_init(void)
{
	...
	ret = platform_driver_register(&twl5031_bcc_driver);
	return ret;
}
