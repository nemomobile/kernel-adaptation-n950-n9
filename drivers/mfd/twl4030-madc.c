/*
 * drivers/i2c/chips/twl4030-madc-bme.c
 *
 * TWL4030 MADC module driver for Nokia BME
 *
		...
		u8 reg_base, u16 ch_mask, u16 *buf)
{
	...
	ret = twl_i2c_read(TWL4030_MODULE_MADC, (u8 *)&buf[first_ch],
...
static irqreturn_t twl4030_madc_threaded_irq_handler(int irq, void *_madc)
{
	...
	for (i = 0; i < TWL4030_MADC_NUM_MODES; i++) {
		...
		if (i == TWL4030_MADC_MODE_RT) {
			...
			disable_buf[1] = 0;
			disable_buf[2] = 0;
			twl_i2c_write(TWL4030_MODULE_MADC, disable_buf,
					  mode->sel, 2);
		}
					...
					u16 mode_id, int force_setup)
{
	...
	if (setup_length) {
		...
		ret = twl_i2c_write(TWL4030_MODULE_MADC, setup_buf,
...
static void twl4030_madc_hybrid_finish(struct work_struct *work)
{
	...
	if (the_madc->active[TWL4030_MADC_MODE_RT] == req) {
		...
		ret = twl_i2c_read(TWL4030_MODULE_MADC, disable_buf,
			twl4030_conversion_modes[TWL4030_MADC_MODE_RT].sel, 2);
...
static int __devinit twl4030_madc_probe(struct platform_device *pdev)
{
	...
	ret = twl_i2c_read_u8(TWL4030_MODULE_INTBR, &regval,
				  TWL4030_INTBR_GPBR1);
	if (ret) {
