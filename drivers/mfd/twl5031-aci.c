...
struct twl5031_aci_data {
	...
	int			(*hw_plug_set_state)(struct device *dev,
						bool plugged);
	struct pm_qos_request 	qos_request_acicmd;
	struct pm_qos_request 	qos_request_accint;
};
...
static int twl5031_aci_cmd(u8 task)
{
	...
	pm_qos_add_request(&the_aci->qos_request_acicmd, PM_QOS_CPU_DMA_LATENCY, 0);
...
void static twl5031_read_buttons(struct twl5031_aci_data *aci, bool latched)
{
	...
	pm_qos_add_request(&aci->qos_request_accint, PM_QOS_CPU_DMA_LATENCY, 0);
...
static int __devinit twl5031_aci_probe(struct platform_device *pdev)
{
...
static int __devexit twl5031_aci_remove(struct platform_device *pdev)
{
	struct twl5031_aci_platform_data *pdata = pdev->dev.platform_data;
